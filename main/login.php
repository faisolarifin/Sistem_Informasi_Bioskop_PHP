...
<?php
  if (@$_POST['login'])
  {
    $email = $konek->real_escape_string(htmlentities($_POST['email']));
    $password = $konek->real_escape_string(htmlentities($_POST['password']));  

    $query = $konek->query("SELECT * FROM user WHERE email='$email' AND password='$password' AND hak_akses != 'customer'");
    if ($query->num_rows > 0) 
    {
